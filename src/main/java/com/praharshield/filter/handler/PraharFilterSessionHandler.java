...
package com.praharshield.filter.handler;
...
import com.praharshield.filter.protocol.data.EntityMetadata;
import com.praharshield.filter.protocol.data.ItemFrame;
...
public class PraharFilterSessionHandler implements LimboSessionHandler {
  ...
  public PraharFilterSessionHandler(Player proxyPlayer, PraharFilter plugin) {
  ...
  private void fallingCheckFailed(String reason) {
    if (Settings.IMP.MAIN.FALLING_CHECK_DEBUG) {
      PraharFilter.getLogger().info(reason);
      this.logPosition();
    }
  ...
  public void onGeneric(Object packet) {
    if (packet instanceof PluginMessagePacket) {
      ...
      if (PluginMessageUtil.isMcBrand(pluginMessage) && !this.checkedByBrand) {
        ...
        PraharFilter.getLogger().info("{} has client brand {}", this.proxyPlayer, brand);
  ...
  private boolean checkPing() {
    ...
    if (Settings.IMP.MAIN.TCP_LISTENER.PROXY_DETECTOR_ENABLED && (l7Ping - l4Ping) > Settings.IMP.MAIN.TCP_LISTENER.PROXY_DETECTOR_DIFFERENCE) {
      ...
      if (Settings.IMP.MAIN.TCP_LISTENER.DEBUG_ON_FAIL) {
        PraharFilter.getLogger().info("{} failed proxy check: L4 ping {}, L7 ping {}", this.proxyPlayer, l4Ping, l7Ping);
      }
    ...
    if (Settings.IMP.MAIN.TCP_LISTENER.DEBUG_ON_SUCCESS) {
      PraharFilter.getLogger().info("{} passed proxy check: L4 ping {}, L7 ping {}", this.proxyPlayer, l4Ping, l7Ping);
    }
