package com.praharshield.filter.cache;
...
public class CachedPackets {
    ...
    packet
        .prepare(new LegacyChatPacket(
            ProtocolUtils.getJsonChatSerializer(ProtocolVersion.MINIMUM_VERSION).serialize(
                PraharFilter.getSerializer().deserialize(text)
        ...
        .prepare(version -> new SystemChatPacket(
            new ComponentHolder(version, PraharFilter.getSerializer().deserialize(text)), ChatType.SYSTEM
  ...
  private PreparedPacket createDisconnectPacket(LimboFactory factory, String message) {
    return factory.createPreparedPacket().prepare(version ->
        DisconnectPacket.create(PraharFilter.getSerializer().deserialize(message), version, StateRegistry.PLAY)).build();
  }
  ...
  public void createTitlePacket(PreparedPacket preparedPacket, String title, String subtitle) {
    preparedPacket.prepare(version -> {
      ...
      packet.setComponent(new ComponentHolder(version, PraharFilter.getSerializer().deserialize(title)));
      return packet;
    }, ProtocolVersion.MINECRAFT_1_8);
    ...
    if (!subtitle.isEmpty()) {
      preparedPacket.prepare(version -> {
        ...
        packet.setComponent(new ComponentHolder(version, PraharFilter.getSerializer().deserialize(subtitle)));
        return packet;
      }, ProtocolVersion.MINECRAFT_1_8);
    }
