...
package com.praharshield.filter;
...
import com.praharshield.filter.cache.CachedPackets;
import com.praharshield.filter.captcha.CaptchaGenerator;
import com.praharshield.filter.captcha.CaptchaHolder;
import com.praharshield.filter.commands.PraharFilterCommand;
import com.praharshield.filter.commands.SendFilterCommand;
import com.praharshield.filter.handler.PraharFilterSessionHandler;
import com.praharshield.filter.listener.FilterListener;
import com.praharshield.filter.listener.TcpListener;
import com.praharshield.filter.protocol.packets.Interact;
import com.praharshield.filter.protocol.packets.SetEntityMetadata;
import com.praharshield.filter.protocol.packets.SpawnEntity;
...
public class PraharFilter {
  ...
  @Inject
  public PraharFilter(Logger logger, ProxyServer server, Metrics.Factory metricsFactory, @DataDirectory Path dataDirectory) {
    setLogger(logger);
    INSTANCE = this;
  ...
  public void onProxyInitialization(ProxyInitializeEvent event) {
    ...
    try {
        if (!UpdatesChecker.checkVersionByURL("https://raw.githubusercontent.com/PraharShield/PraharFilter/master/VERSION", Settings.IMP.VERSION)) {
            LOGGER.error("****************************************");
            LOGGER.warn("A PraharFilter update is available, please update.");
            LOGGER.error("https://github.com/PraharShield/PraharFilter/releases/");
            LOGGER.error("****************************************");
        }
    } catch (Exception ignored) {
        // No public version endpoint yet — suppress 404 noise
    }
  ...
  public void reload() {
    ...
    PraharFilterSessionHandler.setFallingCheckTotalTime(Settings.IMP.MAIN.FALLING_CHECK_TICKS * 50L);
    ...
    manager.register("limbofilter", new PraharFilterCommand(this), "lf", "botfilter", "bf", "lfilter");
  ...
  public void sendToFilterServer(Player player) {
    try {
      ...
      this.filterServer.spawnPlayer(player, new PraharFilterSessionHandler(player, this));
    } catch (Throwable t) {
      t.printStackTrace();
    }
  }
  ...
  public static PraharFilter getInstance() {
    return INSTANCE;
  }
