...
package com.praharshield.filter.commands;
...
public class PraharFilterCommand implements SimpleCommand {

  private static final List<UUID> PLAYERS_WITH_STATS = new ArrayList<>();

  private static final List<Component> HELP_MESSAGE = List.of(
      Component.text("This server is using PraharFilter and LimboAPI.", NamedTextColor.YELLOW),
      Component.text("Based on LimboFilter by Elytrium (https://elytrium.net/)", NamedTextColor.YELLOW),
      Component.text("Prahar Shield Edition (https://praharshield.com/)", NamedTextColor.YELLOW),
      Component.empty()
  );
  ...
  public PraharFilterCommand(PraharFilter plugin) {
    this.plugin = plugin;
    ...
    Serializer serializer = PraharFilter.getSerializer();
  ...
  public boolean hasPermission(Invocation invocation) {
    return Settings.IMP.MAIN.COMMAND_PERMISSION_STATE.HELP
        .hasPermission(invocation.source(), "praharshield.commands.help");
  }
  ...
  private Component createStatsComponent(InetAddress address, long ping) {
    ...
    return PraharFilter.getSerializer().deserialize(
  ...
  private enum Subcommand {
    RELOAD("Reload config.", Settings.IMP.MAIN.COMMAND_PERMISSION_STATE.RELOAD,
        (PraharFilterCommand parent, CommandSource source, String[] args) -> {
    ...
    public boolean hasPermission(CommandSource source) {
      return this.permissionState.hasPermission(source, "praharshield.admin." + this.command);
    }

    public Component getMessageLine() {
      return Component.textOfChildren(
          Component.text("  /praharshield " + this.command, NamedTextColor.GREEN),
  ...
  private interface SubcommandExecutor {
    void execute(PraharFilterCommand parent, CommandSource source, String[] args);
  }
}
